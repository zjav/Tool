package com.ulane.ulink.im.util;
...
public class SolarTermsUtil {
    ...
    public static void main(String[] args) {
        LocalDate of = LocalDate.of(2023, 1, 22);
        ZoneId zoneId = ZoneId.systemDefault();
        Date date = Date.from(of.atStartOfDay().atZone(zoneId).toInstant());

//        SolarTermsUtil l = new SolarTermsUtil(System.currentTimeMillis());
        SolarTermsUtil l = new SolarTermsUtil(date);
        ...
        String lFestivalName1 = l.getLFestivalName();
        String sFestivalName1 = l.getSFestivalName();
        System.out.println("l "+lFestivalName1);
        System.out.println("s "+sFestivalName1);
...
//        }
     ...
     * @return 日期(数字,所在月份的第几天)
     * solarYear 表示公历年份；
     * solarTermInfo 是一个数组，存储了 24 个节气在当年中所对应的月份和日期；
     * index 表示节气的序号，从 0 开始；
     * 31556925974.7 是每个世纪的毫秒数，即 100 * 365.2422 * 24 * 60 * 60 * 1000；
     * 60000L 表示一分钟的毫秒数。
     * 该公式先根据公历年份 solarYear 计算出从 1900 年到该年的毫秒数，再加上该年当前节气的毫秒数，就得到了该节气在 UTC 时间下的时间戳。
     */
